...
To do this, we recommend creating the λFS "primary" client and experimental driver virtual machine using the `create_aws_infrastructure.py` script. Alternatively, you can create and deploy an EC2 VM yourself using the AMI `ami-0afdef1cdfdd085c3`. Ensure this virtual machine is running before connecting to the instance via SSH. 
