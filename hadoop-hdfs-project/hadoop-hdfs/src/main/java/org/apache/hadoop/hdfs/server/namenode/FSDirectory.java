...
//  }
  ...
  public INodesInPath getINodesInPath(String path, boolean resolveLink) throws UnresolvedLinkException, StorageException,
    ...
    for (short i = 0; i < paths.length; i++) {
      ...
      boolean cached = namesystem.getMetadataCache().containsKey(partitionId);

      partitionId = INode.calculatePartitionId(partitionId, component, i);

      // This line looks gross but was auto-generated by the IDE. It just avoids using the '+' operator too much.
      builder.append("Path: ").append('"').append(component).append('"')
              .append(", INode already in cache: ").append(cached);
    }
    ...
    partitionId = INode.calculatePartitionId(HdfsConstantsClient.GRANDFATHER_INODE_ID,
            INodeDirectory.ROOT_NAME, INodeDirectory.ROOT_DIR_DEPTH);
