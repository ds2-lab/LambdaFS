...
public class TestResolveHdfsSymlink {
  ...
  public void testFcResolveAfs() throws IOException, InterruptedException {
    ...
    Path alphaHdfsPathViaLink = new Path(
        fcHdfs.getDefaultFileSystem().getUri().toString() + "/tmp/link/alpha");

    Set<AbstractFileSystem> afsList =
        fcHdfs.resolveAbstractFileSystems(alphaHdfsPathViaLink);
