...
public class TestSafeMode {
  ...
  public void startUp() throws IOException {
    ...
    conf.set(HdfsClientConfigKeys.Retry.POLICY_SPEC_KEY, "1000,5");
  ...
  /**
   * This test verifies that if SafeMode is manually entered, name-node does
   * not
  ...
  public void testManualSafeMode() throws IOException {
    ...
    cluster =
        new MiniDFSCluster.Builder(conf).numDataNodes(0).format(false).build();
    cluster.waitActive();
  ...
  public void testNoExtensionIfNoBlocks() throws IOException {
    cluster.getConfiguration(0)
        .setInt(DFSConfigKeys.DFS_NAMENODE_SAFEMODE_EXTENSION_KEY, 60000);
  ...
  public void testInitializeReplQueuesEarly() throws Exception {
    ...
    BlockManagerTestUtil
        .setWritingPrefersLocalNode(cluster.getNamesystem().getBlockManager(),
            false);
    ...
    cluster.getConfiguration(0)
        .setFloat(DFSConfigKeys.DFS_NAMENODE_REPL_QUEUE_THRESHOLD_PCT_KEY,
            1f / 15f);
  ...
  public void testRbwBlocksNotConsideredUnderReplicated() throws IOException {
    ...
    try {
      ...
      DFSTestUtil
          .createFile(fs, new Path("/junk-blocks"), BLOCK_SIZE * 4, (short) 1,
              1L);
      ...
      for (int i = 0; i < 10; i++) {
        FSDataOutputStream stm =
            fs.create(new Path("/append-" + i), true, BLOCK_SIZE, (short) 1,
                BLOCK_SIZE);
        stms.add(stm);
        stm.write(1);
        stm.hflush();
   ...
   */
  @Test(timeout = 300000)
  public void testOperationsWhileInSafeMode()
